...
import it.tweetmail.server.utils.PlatformUtil;
...
class ObservablesManagerImpl<T> implements ObservablesManager<T> {
    ...
    @Override
    public void newObject(T name) { obObjs.putIfAbsent(name, new SimpleObjectProperty<>()); }

    @Override
    public void newList(T name) { obLists.putIfAbsent(name, FXCollections.observableArrayList()); }
